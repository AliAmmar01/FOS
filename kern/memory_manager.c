...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)	//called by sys_allocateMem() called in malloc() in uheap.c
{
	//TODO: [PROJECT 2023 - MS2 - [2] User Heap] allocateMem() [Kernel Side]
	// Write your code here, remove the panic and write your code
	//panic("allocateMem() is not implemented yet...!!");

	int blocks_needed = ROUNDUP(size, PAGE_SIZE)/PAGE_SIZE;	//translate the passed size into an int number of blocks 
	for(int i = 0; i < blocks_needed; i++) {
		int ret = pf_add_empty_env_page(e, virtual_address, 0);	//add a new environment with the given virtual address to the page file and initialize it by zero (e is added to the page file and initialized by 0)
		if(ret != E_NO_PAGE_FILE_SPACE){	//check if there is enough available space in the page file
			virtual_address += PAGE_SIZE;	//increment the passed virtual address by PAGE_SIZE (virtual address to equal next block's virtual address)
		}
	}
...
void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	int blocks_needed = ROUNDUP(size, PAGE_SIZE)/PAGE_SIZE;	//translate the passed size into an int number of blocks, used to get the range
	uint32 VA = virtual_address;	//used to hold the passed virtual address so that we can make changes to it

	//This function should:
	//1. Free ALL pages of the given range from the Page File

	for(int i = 0; i < blocks_needed; i++) {
		pf_remove_env_page(e, VA);	//removes the existing environment page at VA from the page file.
		VA += PAGE_SIZE;	//increment the passed virtual address by PAGE_SIZE (virtual address to equal next block's virtual address)
	}

	//2. Free ONLY pages that are resident in the working set from the memory

	for(int i = 0; i < e->page_WS_max_size; i++)	//for loop on all e's working set page 
	{
		if(e->ptr_pageWorkingSet[i].virtual_address >= virtual_address && e->ptr_pageWorkingSet[i].virtual_address < (virtual_address + size))	//check if e's page working set entry virtual address is within the passed virtual address range
		{
			uint32* ptr_page_table = NULL;
			struct Frame_Info* ptr_frame_info = get_frame_info(e->env_page_directory, (void*)e->ptr_pageWorkingSet[i].virtual_address, &ptr_page_table);	//used to get both the page table and the frame of the given virtual address, returns a pointer to it
			if(ptr_frame_info != NULL)	//check if the returned pointer points to an actual frame
			{
				unmap_frame(e->env_page_directory, (void*) e->ptr_pageWorkingSet[i].virtual_address);	//used to un-map a frame at the given virtual address, simply by clearing the page table entry (-The references count on the physical frame should decrement.- The physical frame should be freed if the 'references' reaches 0.- The page table entry corresponding to 'virtual_address' should be set to 0.(if such a page table exists)- The TLB must be invalidated if you remove an entry fromthe page directory/page table.)
				pt_clear_page_table_entry(e, e->ptr_pageWorkingSet[i].virtual_address);	//set the entry of the given page inside the page table to NULL. This indicates that the page no longer exists in the memory.
				env_page_ws_clear_entry(e, i);	//Clears (make empty) the entry at i in e working set.
			}
		}
	}
	...
	VA = virtual_address;	//VA is initialized back to virtual_address since it was changed
	for(int i = 0; i < blocks_needed; i++)	//for loop for blocks_needed iterations
	{
		uint32* ptr_page_table = NULL;
		struct Frame_Info* ptr_frame_info = get_frame_info(e->env_page_directory, (void*)VA, &ptr_page_table);	//used to get both the page table and the frame of the given virtual address, returns a pointer to it
		if(ptr_frame_info != NULL)	//check if the returned pointer points to an actual frame
		{
			if((pt_get_page_permissions(e, VA) &  PERM_BUFFERED) && (pt_get_page_permissions(e, VA) & PERM_MODIFIED))	//check if the e's page is buffered and modified
			{
				bufferlist_remove_page(&modified_frame_list, ptr_frame_info);	//removes ptr_frame_info from the modified_frame_list 
				ptr_frame_info->isBuffered = 0;	//set isBuffered to 0 to flag it as free frame
				ptr_frame_info->environment = NULL;	//set environment to NULL to flag it as free frame
				free_frame(ptr_frame_info); //add the frame to the free_frame_list
			}
			else if((pt_get_page_permissions(e,VA) & PERM_BUFFERED) && (!(pt_get_page_permissions(e,VA) & PERM_MODIFIED)))	//check if the e's page is buffered and not modified
			{
				ptr_frame_info->isBuffered = 0;	//set isBuffered to 0 to flag it as free frame
				ptr_frame_info->environment = NULL;	//set environment to NULL to flag it as free frame
				bufferlist_remove_page(&free_frame_list, ptr_frame_info);	//removes ptr_frame_info from the free_frame_list
				free_frame(ptr_frame_info);	//add the frame to the free_frame_list
			}
			pt_clear_page_table_entry(e, VA);	//set the entry of the given page inside the page table to NULL. This indicates that the page no longer exists in the memory
		}
		VA += PAGE_SIZE;	//increment the passed virtual address by PAGE_SIZE (virtual address to equal next block's virtual address)
	}
	...
	for(int i = 0; i < blocks_needed; i++)	//for loop for blocks_needed iterations
	{
		bool used = 0;
		uint32* ptr_page_table = NULL;
		int ret = get_page_table(e->env_page_directory, (void*) VA, &ptr_page_table);	//returns a pointer to the page table if it exists 
		if(ret != TABLE_NOT_EXIST)	//check if the returned value doesn't mean that trhe table wasn't found 
		{
			int j = 0;
			while(j < 1024)	//while loop to loop on all page table entries
			{
				if(ptr_page_table[j] != 0)	//when it encounters an entry that isn't equal to 0
				{
					used = 1;	//flag the table as used
					break;
				}
				j++;
			}
			if(used == 0)	//if after iterating on all page table entries the table is still unused
			{
				kfree((void*)ptr_page_table);	//kfree function call to free the frame containing the page table
				pd_clear_page_dir_entry(e, VA);	//set the entry of the page table inside the page directory to NULL. This indicates that the page table, which contains VA, no longer exists in the whole system (memory and page file).
			}
		}
		VA += PAGE_SIZE;	//increment the passed virtual address by PAGE_SIZE (virtual address to equal next block's virtual address)
	}
